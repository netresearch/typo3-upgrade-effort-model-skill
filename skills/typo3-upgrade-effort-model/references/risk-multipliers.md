...
| Magic Extbase repo finders | grep `findBy[A-Z]\w+` | × 1.1 | Mostly mechanical via Rector. Counts ONLY for true `__call` magic finders — a self-defined `findBy*` repository method using `createQuery()` is not affected, do not apply |
| Core asset concat/compression removal | grep `config.concatenateCss` / `concatenateJs` | × 1.2 | Vite/webpack integration if no build tool yet. If a build tool (Vite/webpack) is already present, this is a config migration only — use the lower end. Breaking [#108055](https://docs.typo3.org/c/typo3/cms-core/main/en-us/Changelog/14.0/Breaking-108055-RemovedFrontendAssetConcatenationAndCompression.html) |
